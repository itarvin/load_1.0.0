...
class Base extends Controller
{
    ...
    protected function initialize(){
        ...
        // 分割地址和协议
        $url = $origins ? parse_url($_SERVER['HTTP_ORIGIN']) : '';
        // 跨域标识
    ...
    public function returnRes($AuthPermission, $isCheck = 'false')
    {
        if( $isCheck == 'true'){
            if( $AuthPermission == '400'){
                $res = [ReturnCode::ACCOUNTEXPIRED, "请您先登录账户！"];
            }else if( $AuthPermission == '300'){
                $res = [ReturnCode::ACCOUNTEXPIRED, Tools::errorCode(ReturnCode::ACCOUNTEXPIRED)];
            }
        }
        return $res;
    }
